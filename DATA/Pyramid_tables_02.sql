/* Bit array explanation */
-- bits from right to left, every bit as a flag, used for example in column [reported] in table Pyramid
-- example:
...
SELECT TOP 10000
  ...
  ,reported -- > 0 if asset has been submitted (bit array with two bits: 1 = Asset Group Correction, 2 = Serial Number Corection, 3 = 1 + 2 = Both of them)
				-- parents will have always 3 (because they are included only in Investigation file)
  ...
  dtActive -- date when the record has been present in data load source (set also for ignored records), this is needed, 
			-- because we are not receiving terminated assets from HPFS.
